#include "Bc2Dmunu/BasicFitter.h"
...
void Bc2Dmunu::BasicFitter::makePlots(){
  ...
  PlotComponent pc_oth( "pdf:comb_pdf,bu2dstmunu_mc_pdf", "Other" );
  pc_oth.setSolidFill(kGreen-3);
...
void Bc2Dmunu::BasicFitter::makeTotalPdf() {
  ...
  addParameter("oth_y", 0, 500e4);
  w->factory( "SUM::pdf( bkg_y * comb_pdf, sig_y * bu2dmunu_mc_pdf, oth_y * bu2dstmunu_mc_pdf)" );
  defineParamSet("pdf");
  defineYieldSet("pdf");
}
